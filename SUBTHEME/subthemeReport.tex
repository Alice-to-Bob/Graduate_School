...
\begin{comment}
    ...
    \begin{table}[htb]
        \begin{tabular}{|c|c|}\hline
            ...
            テレポー�����ノード&テレポートサービスを実行しているノード。許可されたテレポートユーザがアクセスできる。\\ \hline
...
\begin{figure}[H]
    \centering
    \includegraphics*[width=1.0\textwidth,page=2]{graphs/softetherVPN.pdf}
    \caption{リモートアクセスVPN}
    \label{softether}
\end{figure}
...
サーバー側に、サービスファイル(/etc/systemd/system/vpnclient.service)を定義する。
\begin{lstlisting}
[Unit]
...
設定はGUIの管理ツール「SoftEther VPN Server Manager」か、コマンドラインツールの「vpncmd」を使って行う。
今回はコマンドラインツール「vpncmd」を使って設定した。ネットワークが繋がっていれば、vpncmdを使ってリモート設定も行える。
...
\begin{description}
    ...
    \item[\$] sudo ipconfig set tap0 dhcp \\(ここからvpncmdを抜けて、コマンドラインで行う。)
    \item[\$] ifconfig tap0 \\
    \begin{figure}[H]
        \centering
