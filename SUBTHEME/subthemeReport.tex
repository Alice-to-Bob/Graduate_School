...
この章では、通信接続やその接続の管理を容易にするために必要な要素を考える。

自分が考えるものを以下に挙げる。

\begin{itemize}
    \item SSHを行う回数（段数）を減らすこと。
    \item SSHの際のトンネルを作る、管理できる。%トンネリング
    \item 利用端末と遠隔アクセス端末側のみアカウントがあれば利用できる。%VPN
...
今回試用したソフトウェアの中では、「Teleport」はトンネリングを行っている。「Teleport」では、認証を行ったあと隔離サーバとGateway
間でトンネルが引かれている。
...
システム管理者にとって最も面倒な作業だと感じる瞬間は、一括管理されていないPCやサーバに新規ユーザ登録の時であろう。
...
「sshportal」は、「LDAP」等の技術を利用していないがデータベースにアカウント情報を登録することでユーザのログイン
...
\chapter{まとめ}
本論文で前章までで、遠隔アクセスのために使われる技術とそのセキュリティ性向上のために使われている技術を解説し、
その後それらの技術を用いているいくつかのオープンソースソフトウェアを試用してきた。ここでは全体の総括とまとめを行う。
\if0
...
\fi
%\subsubsection*{認証技術}
%ここでは、代表的な認証技術として「LDAP」、「Kerberos」、「Radius」、「Active Directory」を解説した。\par
%「LDAP」は、
