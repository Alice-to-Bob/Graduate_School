...
\begin{abstract}
    ...
    また、令和２年新型コロナウィルスの拡大によりテレワークが普及し、遠隔地から社内ネットワークにアクセスすること
    が多くなり、リモートアクセスにおけるセキュリティが重要になっている。


    そこで、本研究ではリモートアクセスの根底にある技術を解説するとともに、    
    サーバーへの通信制御に有効なアプリケーションを調査することを目的とする。
    そのために、3台のサーバーと1台のL2スイッチを用いて隔離ネットワークを構成し、その３台のサーバーに
    個人ソースコード投稿サイトに公開されている
    通信制御のアプリケーションを試用し、評価を行う。 
    
    
    %ゲートウェイサーバーに個人のソースコード投稿サイトに公開されている
    %通信制御のアプリケーションを試用した後、検討を行う。
\end{abstract}
...
そのような通信制御のアプリケーションなしでは、システム管理者はユーザーの接続ごとに手動でコマンドを実行しなければならず、
サーバーを利用するユーザーが多くなるに従いシステム管理者への負担が多くなってしまう。\par
...
\begin{enumerate}
    ...
    \setlength{\itemsep}{0.1cm} 
    \item 接続段数を減らしたい。
    \item 接続の際のトンネルの管理を容易にしたい。%トンネリング
    \item ログイン時に入力の手間を省きたい。%証明書
    \item ユーザ情報の一括管理を行たい。%LDAP,Kerberos
    \item 初期登録時、一括管理に加えて登録の自動化を行えるようにしたい。
    \item GUIが利用したい。
    \item 接続ログやセッションログが取りたい。
    \item 利用端末と遠隔アクセス端末側(E2E)のみアカウントがあれば利用したい。%VPN
\end{enumerate}
